...
export default function AdminQuizzes() {
  ...
  const load = async () => {
    try {
      ...
      setQuizzes(qzRes.data); setCategories(cRes.data); setUsers(uRes.data)
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      quizRuleRequests: form.quizRuleRequests.map(r => ({
        ...r, categoryId: Number(r.categoryId), numQuestions: Number(r.numQuestions)
      }))
    }
    try {
      await quizAPI.create(payload); showToast('Tạo quiz thành công')
      setModal(false); load()
  ...
  const handleDelete = async () => {
    try {
      await quizAPI.delete(confirm.id); showToast('Xóa quiz thành công')
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                    {q.quizRuleResponses.map((r, i) => (
                      <span key={i} style={{
                        ...
                        borderRadius: 99, fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)'
                      }}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <input className="form-input" required value={form.title}
                  onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Tên bài thi..." />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div className="form-group">
                  <label className="form-label">Thời gian (phút) *</label>
                  <input className="form-input" type="number" min={1} required value={form.duration}
                    onChange={e => setForm({ ...form, duration: e.target.value })} />
                </div>
                <div className="form-group">
                  <label className="form-label">Người tạo *</label>
                  <select className="form-select" required value={form.userId}
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {form.quizRuleRequests.map((r, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto auto', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                    <select className="form-select" required value={r.categoryId}
                      onChange={e => updateRule(i, 'categoryId', e.target.value)}>
                      <option value="">Danh mục</option>
                      {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <select className="form-select" value={r.difficulty}
                      onChange={e => updateRule(i, 'difficulty', e.target.value)}>
                      {DIFFICULTIES.map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                    <input className="form-input" type="number" min={1} style={{ width: 70 }} value={r.numQuestions}
                      onChange={e => updateRule(i, 'numQuestions', e.target.value)} title="Số câu" />
        ...
        <div className="modal-overlay" onClick={() => setVariantModal(null)}>
          <div className="modal" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="number" min={1} max={10} value={variantCount}
