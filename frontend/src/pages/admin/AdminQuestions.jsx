...
const emptyForm = () => ({
  content: '', imageUrl: '', difficulty: 'Easy', categoryId: '',
  answers: [
...
function ImageUpload({ value, onChange, small }) {
  ...
  const handleFile = (e) => {
    ...
    // Lưu file vào public/uploads (chỉ hoạt động khi dev, production cần server handle)
    const reader = new FileReader()
    reader.onload = (ev) => {
      // Lưu data URL cho preview và gửi url path lên server
      onChange(url, ev.target.result)
    }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <input
        ...
        style={{ flex: 1 }}
        placeholder="URL ảnh (để trống nếu không có)"
        value={value || ''}
      ...
      {value && (
        <img
          src={value}
          alt="preview"
          style={{ width: small ? 32 : 48, height: small ? 32 : 48, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--border)', flexShrink: 0 }}
          onError={e => e.target.style.display = 'none'}
        />
      )}
      {value && (
        <button type="button" className="btn btn-danger btn-sm" style={{ padding: '4px 8px' }} onClick={() => onChange('', null)}>×</button>
      )}
    </div>
  )
}
...
export default function AdminQuestions() {
  ...
  const handleDelete = async () => {
    try {
      await questionAPI.delete(confirm.id); showToast('Xóa câu hỏi thành công')
  ...
  return (
    <div>
            ...
            <tbody>
              {filtered.map(q => (
                <tr key={q.id}>
                  ...
                  <td>
                    {q.imageUrl
                      ? <img src={q.imageUrl} alt="" style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--border)' }} onError={e => e.target.style.display = 'none'} />
        ...
        <div className="modal-overlay" onClick={() => setModal(false)}>
          <div className="modal" style={{ maxWidth: 620 }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label className="form-label">Hình ảnh câu hỏi (URL)</label>
              ...
              <div className="form-group">
                <label className="form-label">Đáp án (● = đáp án đúng)</label>
                {form.answers.map((a, i) => (
                  <div key={i} style={{ marginBottom: 12, background: 'var(--bg-elevated)', borderRadius: 8, padding: '12px', border: a.isCorrect ? '1px solid rgba(67,233,123,0.4)' : '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8 }}>
                      <input type="radio" name="correct" checked={a.isCorrect} onChange={() => handleAnswerChange(i, 'isCorrect', true)} style={{ accentColor: 'var(--accent)', width: 16, height: 16, flexShrink: 0 }} />
                      <input
                        className="form-input"
                      ...
                      />
                      {a.isCorrect && <span style={{ color: 'var(--success)', fontSize: 16, flexShrink: 0 }}>✓</span>}
                    </div>
                    <ImageUpload small value={a.imageUrl} onChange={(url) => handleAnswerChange(i, 'imageUrl', url)} />
                  </div>
                ))}
              </div>
      ...
      <ConfirmDialog isOpen={!!confirm} message="Bạn có chắc muốn xóa câu hỏi này?" onConfirm={handleDelete} onCancel={() => setConfirm(null)} />
    </div>
  )
}
